...
void Telnet_client_connect(NX_TELNET_SERVER *telnet_server_ptr, UINT logical_connection)
{
  ...
  if ((logical_connection == TELNET_LOGICAL_CONNECTION) && (telnet_server_ptr == &telnet_server))
  {
    ...
    {
      status = Thread_VT100_create((ULONG)&mon_telnet_drv_driver,&telnet_vt100_drv_handle); // В контексте этого вызова будут выполнены функции драйвера init и deinit
      if (status == RES_OK)
      {
