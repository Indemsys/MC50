...
{
  uint32_t res = RES_ERROR;
  res = Serialze_settings_schema_to_JSON_file(&wvar_inst,PARAMS_SCHEMA_FILE_NAME, JSON_ENSURE_ASCII | JSON_COMPACT);
  if (res == RES_OK)
  {
...
{
  uint32_t res = RES_ERROR;
  res = Serialze_settings_to_JSON_file(&wvar_inst,PARAMS_VALUES_FILE_NAME, JSON_ENSURE_ASCII | JSON_COMPACT);
  if (res == RES_OK)
  {
