...
TX_EVENT_FLAGS_GROUP           spi0_flags;
...
//
  reg8 = 0
        + LSHIFT(0, 7) // -         |
        + LSHIFT(0, 6) // SPBYT     | SPI Byte Access Specification.                 0: SPDR is accessed in halfword or word (SPLW is valid)
...
{
  ULONG actual_flags;
  if (SPI0_set_for_hword_transfer_to_display() != RES_OK) return RES_ERROR;
...
uint32_t SPI0_send_buff_to_display(uint16_t *buf, uint32_t sz)
{
  ...
  if (SPI0_set_for_buffer_transfer_to_display() != RES_OK) return RES_ERROR;

  res = tx_event_flags_get(&spi0_flags, FLAG_SPI0_TRANSFER_END, TX_OR_CLEAR,&actual_flags, MS_TO_TICKS(200));
