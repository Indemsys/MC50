...
UINT  ux_hcd_synergy_fifod_write(UX_HCD_SYNERGY *hcd_synergy, UX_SYNERGY_ED *ed)
{
    ...
    if(hcd_synergy->ux_hcd_synergy_transfer.ux_synergy_transfer_tx != 0U)
    {
        ...
        //uint32_t n = Debug_tracing_function(&payload, &fifo);
        //ITM_EVENT32(3, n);
    }
    else
    {
        ...
        //uint32_t n = Debug_tracing_function(&payload, &fifo, __get_PSP(), __get_MSP());
        //ITM_EVENT32(3, n);
    }
                                                 ...
                                                 UX_HCD_SYNERGY_FIFO *fifo)
{
    ...
    if(hcd_synergy->ux_hcd_synergy_transfer_cfg_instance_tx.p_info->length != 0U)
    {
        ...
        hcd_synergy->ux_hcd_synergy_transfer.ux_synergy_transfer_tx->p_api->start(hcd_synergy->ux_hcd_synergy_transfer.ux_synergy_transfer_tx->p_ctrl,TRANSFER_START_MODE_REPEAT);
    ...
    payload->payload_buffer += ((ULONG)hcd_synergy->ux_hcd_synergy_transfer_cfg_instance_tx.p_info->length * (payload->transfer_width) * (payload->transfer_block_count) );

    if(payload->payload_length != 0UL)
    {
         /* Reset the fifo address*/
         fifo->fifo_addr =  (VOID *) (hcd_synergy->ux_hcd_synergy_base + UX_SYNERGY_HC_D0FIFO);
         ux_hcd_synergy_fifo_write_software_copy_remaining_bytes(hcd_synergy, payload->payload_length, payload->payload_buffer, fifo->fifo_addr);
    }
}
