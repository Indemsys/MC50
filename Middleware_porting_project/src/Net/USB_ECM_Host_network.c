...
uint32_t ECM_Host_start_network(void)
{
  ...
  mDNS_start(&mdns,ecm_host_ip_ptr);
...
uint32_t ECM_DHCP_client_start(void)
{
  ...
  else
  {
    ...
    ECM_DEBUG_LOG("ECM IP: %03d.%03d.%03d.%03d Mask: %03d.%03d.%03d.%03d Gateway: %03d.%03d.%03d.%03d", IPADDR(app_net_props.ip_address), IPADDR(app_net_props.network_mask), IPADDR(app_net_props.gateway_address));
