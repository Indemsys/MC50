...
FMSTR_TSA_RW_VAR( wvar.app_type                        ,FMSTR_TSA_UINT8     ) // Applicationl type (0- Servo  BLDC with OPS, 1- DC traction motor, 2- Servo  BLDC, 3 - Async.rot., 4 - Test) | def.val.= 0
FMSTR_TSA_RW_VAR( wvar.bldc_comm_law                   ,FMSTR_TSA_UINT32    ) // Commutation law (6 digit) | def.val.= 264513
FMSTR_TSA_RW_VAR( wvar.bldc_max_rot_speed              ,FMSTR_TSA_FLOAT     ) // Rotor maximum rotation speed (turn/s) | def.val.= 84
FMSTR_TSA_RW_VAR( wvar.bldc_mot_pole_num               ,FMSTR_TSA_UINT8     ) // Rotor pole numbers | def.val.= 8
FMSTR_TSA_RW_VAR( wvar.bldc_sett_max_current           ,FMSTR_TSA_FLOAT     ) // Maximum current during setting (A) | def.val.= 5
