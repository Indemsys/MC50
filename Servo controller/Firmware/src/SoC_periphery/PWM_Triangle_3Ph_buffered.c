...
static void  _GPT0_underflow_isr(void);

SSP_VECTOR_DEFINE_CHAN(_GPT0_underflow_isr, GPT, COUNTER_UNDERFLOW, 0);
...
/*-----------------------------------------------------------------------------------------------------
  ...
  Перезагрузка компаратора происходит при переходе таймера через 0
...
-----------------------------------------------------------------------------------------------------*/
static void _PWM_triangle_buffered_init(R_GPTA0_Type *R_GPT)
{
  ...
  //R_GPT->GTCR_b.MD     = 6;       // 110: Triangle-wave PWM mode 3 (64-bit transfer at trough) (fixed buffer operation)
  //                                // Запись в регитстры производится при переходе счетчика через 0, но обновление компараторов происходит и на пике и на нуле
...
static void _PWM_set_ADC_trigger(void)
{
  ...
  R_GPTA0->GTINTAD_b.ADTRAUEN = 1;     // Compare Match (Up-Counting) A/D Converter Start Request Interrupt Enable
                                       // Разрешаем выдачу сигнала триггера ADC только в фазе нарастания значений счетчика. Чтобы не было двух сигналов от компаратора - при нарастании счетчика и при убывании счетчика
  __DSB();                             // Ожидаем пока все данные будут записаны в периферию
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param f_64tranf  - флаг 64-битной передачи с буфферизацией
...
uint32_t PWM_3ph_triangle_buffered_init(uint32_t freq)
{
  ...
  if (gpt_pwm_cycle_set < PWM_STEP_COUNT)
  {
  ...
  _PWM_triangle_buffered_init(R_GPTA0);
  _PWM_triangle_buffered_init(R_GPTA1);
  _PWM_triangle_buffered_init(R_GPTA2);
...
void Set_output_U(uint8_t mode)
{
  ...
  case FL_PHASE_PULL_UP   :
    ...
    tmp_H = tmp_H | BIT(2) | BIT(0); // Установка 1

    tmp_L = tmp_L & ~(LSHIFT(0x1F,24) | BIT(16) | BIT(0));
    tmp_L = tmp_L | BIT(2);           // Установка 0

    R_PFS->P414PFS = tmp_L;  // Сначала установливаем 0 на нижнем ключе
    ...
    R_PFS->P415PFS = tmp_H;  // Затем установливаем 1 на верхнем ключе
    __DSB();
    break;
  case FL_PHASE_PULL_DOWN :
    // Для фазы U устанавливаем P415 (UH) в 0, P414 (UL) в 1
    tmp_H = tmp_H & ~(LSHIFT(0x1F,24) | BIT(16) | BIT(0));
    tmp_H = tmp_H | BIT(2);          // Установка 0

    tmp_L = tmp_L & ~(LSHIFT(0x1F,24) | BIT(16));
    tmp_L = tmp_L | BIT(2) | BIT(0); // Установка 1

    R_PFS->P415PFS = tmp_H; // Сначала устанавливаем 0 на верхнем ключе
    __DSB();
    R_PFS->P414PFS = tmp_L; // Затем   устанавливаем 1 на нижнем ключе
    __DSB();
    break;
  case FL_PHASE_Z_STATE       :
    ...
    R_PFS->P414PFS = tmp_L;  // Сначала устанавливаем 0 на нижнем ключе
    __DSB();
    R_PFS->P415PFS = tmp_H;  // Затем   устанавливаем 0 на нижнем ключе
    __DSB();
    break;
  case FL_PHASE_PWM_ON       :
    ...
    if (((tmp_H >> 24) & 0x1F) != PSEL_03)
    {
      ...
      tmp_H = tmp_H | BIT(16) | BIT(2) | LSHIFT(PSEL_03,24); // Установка альтернативной функции для верхнего ключа

      tmp_L = tmp_L & ~(LSHIFT(0x1F,24) | BIT(0));
      tmp_L = tmp_L | BIT(16) | BIT(2) | LSHIFT(PSEL_03,24); // Установка альтернативной функции для нижнего ключа
