...
uint32_t GPT0_PWM_init(uint32_t freq)
{
  ...
  R_GPTA0->GTCCRA        = mod - 0x20; // Загружаем начальное значение в компаратор A. Устанавливаем минимальную длительность начального импульса
  R_GPTA0->GTCCRC        = mod - 0x20; // Загружаем буфферизированное начальное значение в компаратор A
  ...
  R_GPTA0->GTADTRA       = mod - 0x10; // Назначаем момент подачи сигнала триггера ADC. В данном случае тригеер сработает ровно по середине импульса ШИМ
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param percents - нормированная к диапазону от -100 до +100 величина модуляции

  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
void GPT0_update_PWM_value(void)
{
  uint32_t mod;
  uint32_t val;

  uint32_t modulation = mot_cbl.pwm_val;
  ...
  if (val < MIM_PWM_IMP_LENGTH)
  {
    val = MIM_PWM_IMP_LENGTH;
  }
  else if ((mod - val) < MIM_PWM_IMP_LENGTH)
  {
    val = mod - MIM_PWM_IMP_LENGTH;
  }
