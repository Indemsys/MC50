...
/*-----------------------------------------------------------------------------------------------------
  Инициализация таймер отсчитывающего переполения для обнаружения остановки вращения ротора

  \param void

  \return uint32_t
-----------------------------------------------------------------------------------------------------*/
static uint32_t GPT_13_Overflow_init(void)
{
  ...
  R_GPTB5->GTPR             = PCLKD_FREQ*2; // Устанавливаем регистр задающий время переполения таймера = 2 сек
...
/*-----------------------------------------------------------------------------------------------------
  ...
  \param void
-----------------------------------------------------------------------------------------------------*/
uint32_t GPT_11_12_Capture_init(void)
{
  R_MSTP->MSTPCRD_b.MSTPD6  = 0;          // Разрешаем работу модулей GPT ch13-ch8

  Reset_rotor_speed_detector();

  GPT_11_Capture_init();
  GPT_12_Capture_init();
  GPT_13_Overflow_init();
