  ...
  #define TMC6200_AMPLIFICATION_5   0  // С утим уровнем усиления возникает искажение на некотором уровне сигнала
...
typedef  struct
{
    union
    {
        struct
        {
            uint32_t  UF         :  1;  // Input Phase Soft Setting W. This bit sets the input phase by the software settings. This bit setting is valid when the OPSCR.FB bit = 1.
            uint32_t  VF         :  1;  // Input Phase Soft Setting V. This bit sets the input phase by the software settings. This bit setting is valid when the OPSCR.FB bit = 1.
            uint32_t  WF         :  1;  // Input Phase Soft Setting U. This bit sets the input phase by the software settings. This bit setting is valid when the OPSCR.FB bit = 1.
            uint32_t             :  1;  //
            uint32_t  U          :  1;  // Input U-Phase Monitor. This bit monitors the state of the input phase.OPSCR.FB=0:External input monitoring by PCLKOPSCR.FB=1:Software settings (UF/VF/WF)
            uint32_t  V          :  1;  // Input V-Phase Monitor. This bit monitors the state of the input phase.OPSCR.FB=0:External input monitoring by PCLKOPSCR.FB=1:Software settings (UF/VF/WF)
            uint32_t  W          :  1;  // Input W-Phase Monitor. This bit monitors the state of the input phase.OPSCR.FB=0:External input monitoring by PCLKOPSCR.FB=1:Software settings (UF/VF/WF)
            uint32_t             :  1;  //
            uint32_t  EN         :  1;  // Enable-Phase Output Control
            uint32_t             :  7;  //
            uint32_t  FB         :  1;  // External Feedback Signal Enable. This bit selects the input phase from the software settings and external input.
...
uint32_t TMC6200_faults_clear_request(void);
